...
# 入力項目
manage_no = st.text_input("管理No")
client = st.text_input("顧客/担当者")
equipment = st.text_input("設備名称")
date_val = st.date_input("作成日")
work_title = st.text_input("内容")
workers = st.text_input("作業者")

if report_type == "トラブル報告書":
    history = st.text_area("１．経緯")
    check_status = st.text_area("２．現場精査")
    investigation = st.text_area("３．調査内容")
    result = st.text_area("４．調査結果")
    action = st.text_area("５．対応内容")
    remarks = st.text_area("６．備考")
...
def edit_excel(template_name):
    ...
    write_cell(ws, "H2", str(date_val))
    write_cell(ws, "C4", client)
